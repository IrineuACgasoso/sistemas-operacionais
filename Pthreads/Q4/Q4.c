// Compilação: gcc -O2 q4.c -o q4 -pthread | Execução: ./q4 <numero_de_threads>
#include <stdlib.h>
#include <stdio.h>
#include <pthread.h>
#include <sys/time.h>

// Define a dimensão do sistema linear e o número de iterações de refinamento do resultado
#define DIMENSIONS 2048
#define INTERACTIONS 1000

// Define a matriz A, o vetor B e o buffer duplo com os valores novo e antigo de x
...
{
    int thread_id;
    int start_idx;      // Índice inicial da linha da thread
    int end_idx;        // Índice final da linha da thread
} ThreadData;

// Função que cada thread executa para resolver seu respectivo trecho do sistema linear
void* solve_jacobi(void* arg) 
{
    ThreadData *data = (ThreadData*)arg;

    // Laço iterativo de Jacobi
    for (int k = 0; k < INTERACTIONS; k++) {
        // Calcula as incógnitas respectivas dessa thread
        for (int i = data->start_idx; i < data->end_idx; i++) {
            ...
            // Isola a incógnita principal da linha
...
int main(int argc, char *argv[]) {
    // Validação dos parâmetros da linha de comando
    if (argc < 2) {
    ...
    // Inicia a matriz A e os vetores
    ...
    // Distribuição da carga de cálculos para cada thread
    int itens_base = DIMENSIONS / numero_threads;
    int remainder = DIMENSIONS % numero_threads; // Resto da divisão
    int current_idx = 0;

    // Cronômetro do tempo de execução
    struct timeval start, end;
    gettimeofday(&start, NULL);

    // Criação das N threads passadas pelo argumento da linha de comando (1, 2 ou 4)
    for (int i = 0; i < numero_threads; i++) {
        thread_data[i].thread_id = i;
        thread_data[i].start_idx = current_idx;
        // Distribui o resto (caso haja) somente entre as primeiras threads
    ...
    // Espera as threads terminarem seus cálculos
    for (int i = 0; i < numero_threads; i++) {
